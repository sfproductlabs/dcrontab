...
const (
	...
	testDBDirName      string = "dcrontab-data"
...
// Commander is a state machine that implements the IOnDiskStateMachine interface.
// Commander stores key-value pairs in the underlying RocksDB key-value store. As
...
// NewCommander creates a new disk kv test state machine.
func NewCommander(clusterID uint64, nodeID uint64) sm.IOnDiskStateMachine {
	d := &Commander{
		clusterID: clusterID,
		nodeID:    nodeID,
	}
...
func (d *Commander) queryAppliedIndex(db *rocksdb) (uint64, error) {
...
func (d *Commander) Open(stopc <-chan struct{}) (uint64, error) {
...
func (d *Commander) Update(ents []sm.Entry) ([]sm.Entry, error) {
	if d.aborted {
	...
	for idx, e := range ents {
		action := &Action{}
		if err := json.Unmarshal(e.Cmd, action); err != nil {
			panic(err)
		}
		wb.Put([]byte(action.Key), []byte(action.Val))
...
func (d *Commander) Sync() error {
	return nil
}

type CommanderCtx struct {
	db       *rocksdb
...
// achieve that.
func (d *Commander) PrepareSnapshot() (interface{}, error) {
	if d.closed {
...
func (d *Commander) saveToWriter(db *rocksdb,
...
func (d *Commander) SaveSnapshot(ctx interface{},
...
func (d *Commander) RecoverFromSnapshot(r io.Reader,
...
func (d *Commander) GetHash() (uint64, error) {
	h := md5.New()
	db := (*rocksdb)(atomic.LoadPointer(&d.db))
